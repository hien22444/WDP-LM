...
import { verifyAccountApi, resendVerification } from "../../services/ApiService";
...
const VerifyAccount = () => {
  ...
  useEffect(() => {
    if (status === "waiting" && resendBtnRef.current && cooldown === 0 && !resendLoading) {
      try {
        resendBtnRef.current.focus();
        resendBtnRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
      } catch {}
    }
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      try {
        ...
        const current = parseInt(window.localStorage.getItem(k) || "0", 10) || 0;
        window.localStorage.setItem(k, String(current + 1));
      } catch {}
    } catch (err) {
      setMessage(
        err?.message || err?.response?.data?.message || "Failed to resend verification email"
      );
      toast.error(
        err?.message || err?.response?.data?.message || "Failed to resend verification email"
      );
            ...
            <div className="verify-status">
              ...
              {email && (
                <button
                  ...
                  ref={resendBtnRef}
                >
                  {resendLoading ? "Resending..." : cooldown > 0 ? `Resend in ${cooldown}s` : "Resend Verification"}
                </button>
              )}
              {!email && (
                <p className="verify-hint">
                  Không tìm thấy email từ bước đăng ký. Vui lòng quay lại trang đăng ký và nhập lại email để nhận liên kết xác minh.
                </p>
              )}
            </div>
          )}
