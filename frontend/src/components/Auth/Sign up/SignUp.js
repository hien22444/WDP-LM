...
const SignUp = () => {
  ...
  const passwordRules = useMemo(() => {
    ...
    const passed = [hasMinLen, hasUpper, hasLower, hasNumber, hasSpecial].filter(Boolean).length;
    const strength = passed >= 5 ? "strong" : passed >= 3 ? "medium" : password.length > 0 ? "weak" : "none";
                ...
                <div className="password-hints">
                  <div className={`strength ${passwordRules.strength}`}>
                    ...
                    <span className="label">
                      {passwordRules.strength === 'strong' ? 'Mạnh' : passwordRules.strength === 'medium' ? 'Trung bình' : passwordRules.strength === 'weak' ? 'Yếu' : ''}
                    </span>
                  </div>
                  <ul className="rules">
                    <li className={passwordRules.hasMinLen ? 'ok' : ''}>Tối thiểu 8 ký tự</li>
                    <li className={passwordRules.hasUpper ? 'ok' : ''}>Có chữ hoa (A-Z)</li>
                    <li className={passwordRules.hasLower ? 'ok' : ''}>Có chữ thường (a-z)</li>
                    <li className={passwordRules.hasNumber ? 'ok' : ''}>Có số (0-9)</li>
                    <li className={passwordRules.hasSpecial ? 'ok' : ''}>Có ký tự đặc biệt (!@#$...)</li>
                  </ul>
                </div>
              </div>
