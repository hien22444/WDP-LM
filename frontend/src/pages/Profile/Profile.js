        ...
        {tutorProfile?.status && (
          <div className={`tutor-status-banner status-${tutorProfile.status}`}>
            <div className="status-info">
              ...
              <strong>
                {tutorProfile.status === "approved" && "Hồ sơ gia sư đã được duyệt"}
                {tutorProfile.status === "pending" && "Hồ sơ gia sư đang chờ duyệt"}
                {tutorProfile.status === "rejected" && "Hồ sơ gia sư bị từ chối"}
                {tutorProfile.status === "draft" && "Bạn chưa hoàn tất đăng ký gia sư"}
              </strong>
              {tutorProfile?.verification?.adminNotes && (
                <span className="admin-notes"> — {tutorProfile.verification.adminNotes}</span>
              )}
            </div>
            <div className="status-actions">
              <button type="button" className="btn-secondary" onClick={() => navigate("/tutor/onboarding")}>Cập nhật hồ sơ gia sư</button>
              {tutorProfile.status === "approved" ? (
                <button type="button" className="btn-primary" onClick={() => navigate("/dashboard")}>Vào bảng điều khiển</button>
              ) : (
                <button type="button" className="btn-primary" onClick={() => navigate("/profile")}>Xem hồ sơ</button>
              )}
            </div>
          </div>
        )}
