    ...
    <div className="home-container">
      ...
      <main>
          ...
          <div className="hero-content">
            <div className="hero-left">
              ...
              <div className="quick-links">
                <button className="q-card" onClick={()=>navigate('/tutor/onboarding')}>
                  <span className="q-icon q1"><i className="fas fa-chalkboard-teacher"></i></span>
                  <div>
                    <div className="fw-bold">Trở thành gia sư</div>
                    <div className="text-secondary" style={{fontSize:12}}>Đăng ký nhanh trong 2 phút</div>
                  </div>
                </button>
                <button className="q-card" onClick={()=>navigate('/tutor')}>
                  <span className="q-icon q2"><i className="fas fa-search"></i></span>
                  <div>
                    <div className="fw-bold">Tìm gia sư phù hợp</div>
                    <div className="text-secondary" style={{fontSize:12}}>Lọc theo môn, hình thức, giá</div>
                  </div>
                </button>
                <button className="q-card" onClick={()=>navigate('/courses')}>
                  <span className="q-icon q3"><i className="fas fa-book-open"></i></span>
                  <div>
                    <div className="fw-bold">Xem khóa học mở</div>
                    <div className="text-secondary" style={{fontSize:12}}>Đặt chỗ ngay trong 1 chạm</div>
                  </div>
                </button>
              </div>
        ...
        <section className="features-section testimonials" data-aos="fade-up">
          <h2 style={{textAlign:'center',marginBottom:24}}>Học viên nói gì về EduMatch</h2>
          <div className="t-grid">
            {[1,2,3].map(i=> (
              <div key={i} className="t-card">
                <div className="t-head">
                  <img className="t-ava" src={`https://i.pravatar.cc/100?img=${10+i}`} alt="avatar"/>
                  <div>
                    <div className="fw-bold">Học viên {i}</div>
                    <div style={{fontSize:12,color:'#64748b'}}>Khóa Toán nâng cao</div>
                  </div>
                </div>
                <div>“Giáo trình rõ ràng, gia sư rất tận tâm. Mình tiến bộ nhanh chỉ sau vài tuần.”</div>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* Partners / Logos */}
        <section className="features-section" data-aos="fade-up" style={{textAlign:'center'}}>
          <h2 style={{marginBottom:18}}>Đối tác đồng hành</h2>
          <div className="logos-strip">
            {[1,2,3,4,5].map(i=> (
              <img key={i} src={`https://dummyimage.com/120x40/ddd/555&text=Logo+${i}`} alt={`logo ${i}`} style={{filter:'grayscale(1)'}}/>
            ))}
          </div>

          {/* FAQ */}
          <div className="faq">
            {[['EduMatch hoạt động thế nào?','Bạn tìm gia sư/khóa học, đặt lịch, học trực tuyến hoặc tại nhà.'],
              ['Chi phí ra sao?','Có nhiều mức giá linh hoạt theo gia sư/khóa học, hiển thị rõ ràng trước khi đặt.'],
              ['Bảo đảm chất lượng?','Gia sư được xét duyệt hồ sơ và đánh giá từ học viên thực, luôn minh bạch.']].map(([q,a],idx)=> (
