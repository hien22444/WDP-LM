...
const CourseDetail = () => {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          if (found) setSlot(found); else setError('Không tìm thấy khóa học');
        } catch (e2) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const list = await BookingService.listPublicTeachingSlots({ tutorId: slot.tutorProfile._id });
        const same = (list || []).filter(s => (s.courseCode && slot.courseCode) ? (s.courseCode === slot.courseCode) : ((s.courseName || '').trim() === (slot.courseName || '').trim()));
        setRelated(same);
      } catch {}
    })();
  }, [slot]);
  ...
  if (error || !slot) return <div className="container py-4 text-danger">{error || 'Không tìm thấy khóa học'}</div>;
    ...
    <div className="container py-4">
      ...
      <div className="mb-3">
        ...
        <div className="d-flex flex-wrap gap-2 mt-2">
          ...
          <span className={`badge rounded-pill ${slot.status === 'open' ? 'text-bg-success' : 'text-bg-secondary'}`}>{slot.status || 'open'}</span>
          {slot.courseCode && <span className="badge rounded-pill text-bg-secondary">Mã: {slot.courseCode}</span>}
        </div>
      </div>
      <div className="row g-4">
            ...
            <div className="card-body">
              ...
              <div className="row g-3">
                <div className="col-12 col-md-6">
                  ...
                  <div className="fw-semibold">{new Date(slot.start).toLocaleDateString()}</div>
                </div>
                <div className="col-12 col-md-6">
                  <div className="small text-secondary">Giờ</div>
                  <div className="fw-semibold">{new Date(slot.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} – {new Date(slot.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                </div>
                <div className="col-12 col-md-6">
                  <div className="small text-secondary">Thời lượng</div>
                  <div className="fw-semibold">{Math.round((new Date(slot.end) - new Date(slot.start)) / (1000*60*60)*10)/10} giờ</div>
                </div>
                {slot.location && (
                  <div className="col-12 col-md-6">
                ...
                <div className="col-12 col-md-6">
                  ...
                  <div className="fw-semibold">{slot.updated_at ? new Date(slot.updated_at).toLocaleString() : new Date(slot.created_at || slot.start).toLocaleString()}</div>
                </div>
                <div className="col-12">
                  <div className="small text-secondary">Tổng số buổi (cùng khóa)</div>
        ...
        <div className="col-12 col-lg-4">
          <div className="card border-0 shadow-sm rounded-4 sticky-top" style={{ top: 16 }}>
            <div className="card-body">
              <div className="d-flex justify-content-between">
                <div className="fw-bold">Giá</div>
                <div className="fw-bold">{slot.price ? slot.price.toLocaleString() : 'Liên hệ'}</div>
              </div>
              <div className="text-secondary">Số học viên: {slot.capacity || 1}</div>
              <div className="mt-3 d-flex align-items-center gap-2">
                <img src={slot.tutorProfile?.user?.avatar || 'https://res.cloudinary.com/dnyvwjbbm/image/upload/v1760334427/whiteava_m3gka1.jpg'} alt="avatar" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover' }} />
                {slot.tutorProfile?.user?.full_name && (
                  <Link to={`/tutor/${slot.tutorProfile._id}`} className="link-dark fw-semibold">{slot.tutorProfile.user.full_name}</Link>
                )}
              </div>
              <div className="d-grid mt-3">
                <button className="btn btn-primary" onClick={() => window.location.assign(`/tutor/${slot.tutorProfile?._id || ''}`)}>Đặt ngay</button>
              </div>
            </div>
          </div>
        </div>
