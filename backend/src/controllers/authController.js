...
const getFirstEnv = (...keys) => {
  for (const k of keys) {
    ...
    if (val !== undefined && val !== null && String(val).trim() !== '') return strip(val);
  }
  return null;
};
...
const resolveGoogleRedirectUri = () => {
  const backendBase = getFirstEnv('BACKEND_PUBLIC_URL') || 'http://localhost:5000';
...
const signRefreshToken = (userId) => {
	return jwt.sign({ sub: userId, type: "refresh" }, process.env.REFRESH_TOKEN_SECRET, {
...
const createTransporter = () => {
	...
	let user = process.env.MAIL_SDN_USERNAME || process.env.MAIL_LEARNMATE_USERNAME || process.env.MAIL_USERNAME;
	let pass = process.env.MAIL_SDN_PASSWORD || process.env.MAIL_LEARNMATE_PASSWORD || process.env.MAIL_PASSWORD;
	...
	if (pass && pass.split(' ').length > 1 && pass.replace(/\s/g, '').length === 16) {
...
const sendMail = async ({ to, subject, html }) => {
  const from = process.env.MAIL_FROM || process.env.MAIL_SDN_USERNAME || process.env.MAIL_LEARNMATE_USERNAME || process.env.MAIL_USERNAME || 'no-reply@learnmate.local';
	try {
...
exports.register = async (req, res) => {
	try {
		...
		const verifyLink = `${process.env.FRONTEND_URL || "http://localhost:3000"}/verify-account?token=${verify_token}`;
...
exports.googleLogin = async (req, res) => {
	try {
		...
		if (!client) return res.status(500).json({ message: 'Google client not configured' });
		let ticket;
		try {
			const aud = resolveGoogleClientId();
			ticket = await client.verifyIdToken({ idToken: credential, audience: aud });
		} catch (e) {
		...
		const email = payload.email;
		const fullName = payload.name || `${payload.given_name || ''} ${payload.family_name || ''}`.trim();
		if (!email) return res.status(400).json({ message: 'Google account has no public email' });
...
exports.verifyAccount = async (req, res) => {
  try {
		...
		const user = await User.findOne({ verify_token: token, verify_token_expires: { $gt: new Date() } });
		if (!user) {
			console.warn('[verifyAccount] token not found or expired', { token });
			return res.status(400).json({ message: 'Invalid or expired token' });
		}
    if (user.status === 'active') return res.json({ message: 'Account already verified' });
    user.status = 'active';
...
exports.resendVerification = async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: 'User not found' });
    if (user.status === 'active') return res.status(400).json({ message: 'Email already verified' });
    // generate new token
    user.verify_token = crypto.randomBytes(40).toString('hex');
    user.verify_token_expires = new Date(Date.now() + 1000 * 60 * 60 * 24);
    await user.save();
    const link = `${process.env.FRONTEND_URL || 'http://localhost:3000'}/verify-account?token=${user.verify_token}`;
    const mailResult = await sendMail({
      to: email,
    ...
    if (!mailResult.ok) return res.status(500).json({ message: 'Could not send email' });
...
exports.resendOTP = async (req, res) => {
	try {
		...
		await user.save();
		await sendMail({ to: email, subject: "Your OTP", html: `<p>OTP: <b>${otp}</b></p>` });
...
exports.verifyOTP = async (req, res) => {
	try {
		...
		const user = await User.findOne({ email });
		if (!user || !user.otp_code) return res.status(400).json({ message: "Invalid OTP" });
...
exports.forgotPassword = async (req, res) => {
	try {
		...
		return res.json({ message: genericMsg, mail: mailResult.mode, mailOk: mailResult.ok });
	} catch (err) {
...
exports.resetPassword = async (req, res) => {
	try {
		...
		const user = await User.findOne({ reset_password_token: token, reset_password_expires: { $gt: new Date() } }).select("+password_hash");
